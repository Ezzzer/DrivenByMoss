...
import de.mossgrabers.controller.launchpad.command.trigger.*;
...
import de.mossgrabers.controller.launchpad.definition.button.LaunchpadButtonInfo;
...
import de.mossgrabers.framework.command.trigger.clip.RecQuantizeCommand;
...
public class LaunchpadControllerSetup extends AbstractControllerSetup<LaunchpadControlSurface, LaunchpadConfiguration> {
    ...
    protected void registerTriggerCommands() {
        ...
        this.addButton(ButtonID.SHIFT, "Shift", new LaunchpadToggleShiftViewCommand(this.model, surface), buttonSetup.get(LaunchpadButton.SHIFT).getControl(),
                () -> transport.isLauncherOverdub() ? LaunchpadColorManager.LAUNCHPAD_COLOR_ROSE : LaunchpadColorManager.LAUNCHPAD_COLOR_RED_AMBER);
        ...
        this.addButton(ButtonID.REC_QUANTIZE, "Rec Quantize", new RecQuantizeCommand<>(this.model, surface), buttonSetup.get(LaunchpadButton.REC_QUANTIZE).getControl(), () -> this.getQuantizeColor(surface, transport));
        ...
        this.addButton(ButtonID.PLAY, "Play", new PlayCommand(this.model, surface), buttonSetup.get(LaunchpadButton.PLAY).getControl(), () -> this.getPlayStateColor(surface));
        this.addButton(ButtonID.NEW, "New", new NewCommand(this.model, surface), buttonSetup.get(LaunchpadButton.NEW).getControl(), () -> this.getNewColor());
        this.addButton(ButtonID.RECORD, "Record", new RecordCommand<>(this.model, surface), buttonSetup.get(LaunchpadButton.RECORD).getControl(), () -> getRecordStsteColor(surface, transport));

        this.addButton(ButtonID.CLIP_REC_AUTOMATION, "Clip Rec Automation", new ClipRecAutomation<>(this.model, surface), buttonSetup.get(LaunchpadButton.CLIP_REC_AUTOMATION).getControl(), () -> getClipRecordAutoStsteColor(surface, transport));
        this.addButton(ButtonID.ARR_REC_AUTOMATION, "Arr Rec Automation", new ArrRecAutomation<>(this.model, surface), buttonSetup.get(LaunchpadButton.ARR_REC_AUTOMATION).getControl(), () -> getArrRecordAutoStsteColor(surface, transport));
        this.addButton(ButtonID.ARR_OVERDUB, "Arr Overdub", new ArrOverdub<>(this.model, surface), buttonSetup.get(LaunchpadButton.ARR_OVERDUB).getControl(), () -> getArrOverdubStsteColor(surface, transport));
        ...
        this.addButton(ButtonID.STOP_ARR, "Stop Arrangement", new StopArrCommand(this.model, surface), buttonSetup.get(LaunchpadButton.STOP_ARR).getControl(), () -> this.getModeColorIndex(ButtonID.STOP_ARR));
    ...
    private int getQuantizeColor(LaunchpadControlSurface surface, ITransport transport) {
        final boolean isShift = surface.isShiftPressed();
        colorIndex++;
        switch (colorIndex) {
            case 0:
                return LaunchpadColorManager.LAUNCHPAD_COLOR_BLUE_LO;
            case 1:
                return LaunchpadColorManager.LAUNCHPAD_COLOR_OCEAN_BLUE;
            case 2:
    ...
    private int getRecordStsteColor(LaunchpadControlSurface surface, ITransport transport) {
        final boolean isShift = surface.isShiftPressed();
    ...
    private int getClipRecordAutoStsteColor(LaunchpadControlSurface surface, ITransport transport) {
        return transport.isWritingClipLauncherAutomation() ? LaunchpadColorManager.LAUNCHPAD_COLOR_ROSE : LaunchpadColorManager.LAUNCHPAD_COLOR_RED_AMBER;
    }

    private int getArrRecordAutoStsteColor(LaunchpadControlSurface surface, ITransport transport) {
        return transport.isWritingArrangerAutomation() ? LaunchpadColorManager.LAUNCHPAD_COLOR_RED_HI : LaunchpadColorManager.LAUNCHPAD_COLOR_RED_LO;
    }

    private int getArrOverdubStsteColor(LaunchpadControlSurface surface, ITransport transport) {
        return transport.isArrangerOverdub() ? LaunchpadColorManager.LAUNCHPAD_COLOR_ROSE : LaunchpadColorManager.LAUNCHPAD_COLOR_RED_LO;
    }
    ...
    private int getDeleteStateColor(final LaunchpadControlSurface surface) {
        ...
        return isDeletePressed(surface) ? LaunchpadColorManager.LAUNCHPAD_COLOR_MAGENTA_HI : LaunchpadColorManager.LAUNCHPAD_COLOR_MAGENTA_LO;
    }

    private boolean isDeletePressed(LaunchpadControlSurface surface) {
        return surface.isPressed(ButtonID.DELETE);
    }
    ...
    private int getModeColorIndex(final ButtonID buttonID) {
        ...
        switch (buttonID) {
            ...
            case STOP_CLIP:
                ...
                return LaunchpadColorManager.LAUNCHPAD_COLOR_GREY_LO;
            case STOP_ARR:
                if (modeManager.isActive(Modes.STOP_ARR))
                    return LaunchpadColorManager.LAUNCHPAD_COLOR_RED_HI;
                return LaunchpadColorManager.LAUNCHPAD_COLOR_RED_LO;
            default:
                return 0;
        }
    }
    ...
    private void handleTrackSelection(final ButtonEvent event, final int index) {
        ...
        if (isDeleteButtonCombination()) {
            //ezer track.remove ();
            track.select();
            model.getApplication().deleteSelection();
            return;
        }

        if (isDuplicateButtonCombination()) {
            track.select();
    ...
    private boolean isDuplicateButtonCombination() {
        return isButtonCombination(ButtonID.DUPLICATE);
    }

    private boolean isDeleteButtonCombination() {
